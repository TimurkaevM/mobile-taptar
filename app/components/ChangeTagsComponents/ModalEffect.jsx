...
const ModalEffect = ({ effectsUser, setEffectsUser }) => {
  ...
  const addEffectFile = () => {
    setEffectsUser([...effectsUser, { ...item, comment }]);
    setComment('');
    closeModal();
  };

  const removeEffectFile = () => {
    const filterEffect = effectsUser.filter((effect) => effect.id !== item.id);
    setEffectsUser(filterEffect);
    setComment('');
    closeModal();
  };

  const changeEffectFile = () => {
    const newEffect = effectsUser.map((effect) => {
  ...
  useEffect(() => {
    if (check) {
      const changeEffect = effectsUser.find((effect) => effect.id === item.id);
